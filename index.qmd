...
ggplot(df_vdem, aes(x = lag_z_cens_mon, y = z_action, color = regime_binary)) +
  geom_point(alpha = 0.5) +  # Optional: adds points to show data
  geom_smooth(method = "lm", se = FALSE) +  # Adds a regression line for each regime type
  labs(
    title = "Figure 5: Impact of Lagged Censorship on Political Action by Regime Type",
    x = "Lagged Censorship (Z-Score)",
    y = "Political Action (Z-Score)",
    color = "Regime Type"
  ) +
...
Moving forward, in order to strengthen consideration of democractic and non-democratic censorship, I would additionally like to utilize some form of matching to compare countries of similar censorship scores in order to consider how extent of social media censorship plays into this conversation.

As it stands, my research points to the potential of an increase usage of social media to organize political action in democracies following increases in social media censorship. Social media censorship in democracies does in fact, operate under different frameworks than it does in non-democracies. I found a stastically significant positive correlation between social media censorship and political actions in democracy, challenging existing literature that argues social media censorship represses political action. While I observed such effects in non-democracies, social media censorship in democracies appears to operate under different frameworks than it does in non-democracies. However, when I controlled for country-level, though my regression still observed a positive trend, it was no longer statistically significant. The increased usage of social media to organize protests could potentially be a backlash to increased restraints on freedom. Democracies, maintaining civic freedoms, might allow for greater criticism of government measures, and individuals might feel more emboldened or entitled to fight for these freedoms. Examining the nature of protests following censorship utilizing a different dataset based on observed instances of protests could provide insight into the causes of this phenomenon. Additionally, analyzing the actual censorship and monitoring practices by looking tangible laws and measures in these nations can help to move the conversation from abstract ideas of censorship and monitoring toward considering specific policies and practices.
